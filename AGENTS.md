...
- WebM → MP4 export uses `ffmpeg-static` with `-c copy` (no re-encode). For MP4 recordings, "Save as" is effectively a file copy. The binary is explicitly listed in `electron-builder` files — do not remove it.
